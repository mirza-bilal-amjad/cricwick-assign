...
const RankOdi = ({route}: any) => {
	const {Layout, darkMode, NavigationTheme, Colors} = useTheme();
	const {colors} = NavigationTheme;

	const {odi} = route.params;
	...
	return (
		<View style={{
			...
			{
				odi && odi.length > 0 && <Animated.FlatList
                    keyExtractor={
						(item, index) => index.toString()
					}
                    showsVerticalScrollIndicator={false}
                    data={odi} renderItem={
					({item, index}: any) => <RenderItem inItem={item} index={index}/>
				}/>
			}
		</View>
	);
}
export default RankOdi
