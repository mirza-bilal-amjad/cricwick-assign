...
const FeaturedContentGHome = ({route}: any) => {
	...
	const returnApiGHome = (pageNumber: number) => {
	...
	const fetch = async () => {
		setRefreshing(true)
		fetchGenericHome(returnApiGHome(pageNumber)).then(
			(r) => {
				if (r) {
				...
				<View style={{
					...
					<View style={{
						...
						<View style={{
							...
							<GoogleIcon name={'visibility'} size={20} color={Colors.textGray400}/>
							<Text
								style={{color: Colors.textGray400}}> {theViews > 1000 ? (theViews / 1000).toFixed(0) + 'k' : theViews}</Text>
						</View>
						<View style={{
							flexDirection: 'row',
							alignItems: 'center'
						}}>
							<GoogleIcon name={'thumb-up'} size={20} color={Colors.textGray400}/>
							<Text style={{color: Colors.textGray400}}> {theLikes}</Text>
						</View>
					</View>
					<TouchableOpacity>
